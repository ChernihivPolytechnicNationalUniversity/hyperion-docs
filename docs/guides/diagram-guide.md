...
<Diagram
  lang="plantuml"
  path="architecture/system-overview.puml"
  alt="System architecture showing main components"
/>
```

**Path Rules**:
- All paths are relative to `apps/docs/diagrams/` directory
...
Store theme files in `apps/docs/diagrams/themes/` directory.
...
- All included files must be within `diagrams/` directory
...
### Path Validation
- All diagram paths must resolve within `diagrams/` directory
...
<Diagram lang="plantuml" path="architecture.puml" alt="Architecture diagram" />

<!-- Graphviz external file -->
<Diagram lang="graphviz" path="flow.dot" alt="Flow diagram" />

<!-- Mermaid inline -->
<Diagram lang="mermaid" chart="graph TD; A-->B;" />

<!-- C4 Model -->
<Diagram lang="c4plantuml" path="c4-architecture.puml" alt="C4 diagram" />
```
