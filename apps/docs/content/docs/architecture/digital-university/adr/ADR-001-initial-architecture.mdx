...
- Must run on university-managed infrastructure (Docker/Kubernetes)
...
#### 4. Low-Code Platform (OutSystems, Mendix)
- **Pros**: Rapid development, less coding
...
```
npp-portal/
├── npp-api/                 # Spring Boot backend
│   └── npp-portal-backend/
│       └── src/main/java/ua/cn/stu/npp/
├── npp-ui/                  # React frontend
│   └── ui/
│       └── src/
...
## References
- [ADR-002: Database Selection](./ADR-002-database-selection)
- [ADR-003: Authentication Strategy](./ADR-003-authentication)
